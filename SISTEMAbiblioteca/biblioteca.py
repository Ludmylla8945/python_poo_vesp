class Biblioteca:
    ...
    def calcularIdadeItem(self, ano_atual):
        ...
        if idade > 70:
            print(f"O livro/revista {self._titulo} é do ano {self._anoPublicacao}, tem idade de {idade} anos e é muito antigo.")
        elif 30 <= idade <= 70:
            print(f"O livro/revista {self._titulo} é do ano {self._anoPublicacao}, tem idade de {idade} anos e é recente.")
        else:
            print(f"O livro/revista {self._titulo} é do ano {self._anoPublicacao}, tem idade de {idade} anos e é contemporâneo.")


class Livro(Biblioteca):
    def verificarTamanho(self):
        if self._numeroPagina > 208:
